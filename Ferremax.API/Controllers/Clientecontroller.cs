...
namespace Ferremax.API.Controllers
{
    ...
    public class Clientecontroller : ControllerBase
    {
        ...
        public dynamic ListarCliente()
        {
            ...
            String conn = "Server=tcp:ferramax-server.database.windows.net,1433;Initial Catalog=ferramax_bd;Persist Security Info=False;User ID=ferremax;Password=Ferre1234*;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout=30;";
            SqlConnection connection = new SqlConnection(conn);
            connection.Open();
            SqlCommand command = new SqlCommand("SELECT * FROM Cliente;", connection);
            SqlDataReader reader = command.ExecuteReader();
            Console.WriteLine("HOLIIIS");
            while (reader.Read()) {
